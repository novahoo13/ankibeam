...
## 九、模板创建流程重设计：字段作为单一来源

> 本节记录一次方向性讨论（2026-02-19）

### 9.1 核心思路转变

| | 现有流程 | 新流程 |
|--|---------|--------|
| Note Type | 用户先去 Anki 手动创建，再回插件填名字 | 用户在插件里定义字段，插件替他在 Anki 里创建 |
| Deck | 用户手动填写已有 Deck 名 | 下拉选已有，或直接输入新名称由插件创建 |
| 字段配置 | 插件侧和 Anki 侧各维护一份，需要人工保持一致 | **单一来源**：插件侧定义一份，派生出 Anki 侧的结构 |

### 9.2 字段的双重身份

用户定义的每个字段同时承担两个角色，只需填写一次：

```
用户定义的字段 { name, label, parseInstruction }
├── 对 Anki 侧 → createModel 的 inOrderFields 的一项（Note Type 结构）
└── 对插件侧 → template.fields[n]（AI 解析规则配置）
```
...
├── Step 2: 检查 Deck
│   └── getDeckNames()
│       ├── 不存在 → createDeck({ deck: deckName })
│       └── 已存在 → 直接复用，无需操作
│
└── Step 3: 保存插件模板
    └── saveTemplate(config, templateData) → chrome.storage
```

> **注意**：Step 1 和 Step 2 要求 AnkiConnect 在线。若离线，插件模板仍可保存，
> 但需提示「Anki 未连接，Note Type 将在首次写入时自动创建」。

### 9.4 字段冲突处理策略

当用户编辑已存在的模板字段时，Anki 侧的 Note Type 可能已有数据：

| 操作 | 风险 | 建议策略 |
|------|------|---------|
| 新增字段 | 低（已有 Note 只是新字段为空） | 自动同步：调用 `modelFieldAdd` |
| 重命名字段 | 中（需映射旧数据） | 警告 + 用户确认，调用 `modelFieldRename` |
| 删除字段 | 高（已有数据永久丢失） | 强警告，默认不执行，需用户明确勾选确认 |
| 字段顺序调整 | 低 | 自动同步：调用 `modelFieldReposition` |

AnkiConnect 提供的相关 API：`modelFieldAdd`、`modelFieldRemove`、`modelFieldRename`、`modelFieldReposition`。

### 9.5 卡片 HTML 自动生成规则

用户不需要手写 Front/Back HTML，插件根据字段列表自动生成默认模板：

```
假设字段为 [Word, Reading, Meaning, Sentence]
...
- 第一个字段作为正面
- 其余字段依次显示在背面
- 高级用户可直接在 Anki 里自定义 HTML/CSS，插件不提供此编辑器

### 9.6 模板列表的 Anki 同步状态 UI

模板列表中每条记录需要显示与 Anki 的同步状态：

```
✅ 已同步     — Note Type 存在且字段一致
⚠️  未安装     — Note Type 不存在（首次写入时自动创建）
🔄 字段不一致  — Note Type 存在但字段与插件配置不匹配
❌ 无法检查   — AnkiConnect 未连接
```

> **TODO**: 确认此状态是实时检查（每次打开 Options 都查询）还是缓存（上次同步时间戳）。
...
- [ ] `createModel` 对同名 Model 的行为测试（报错还是静默忽略？）
- [ ] 悬浮面板是否有模板切换器 UI？
- [ ] **Deck 绑定方案确认**（选项 A 绑定专属 Deck / 选项 B 用户自填）
- [ ] 内置模板的 CSS 和卡片 HTML 设计（需要 UI 设计决策）
- [ ] **字段冲突同步策略确认**（自动同步 vs 手动确认，各操作类型分别如何处理）
- [ ] **Anki 同步状态检查时机**（实时查询 vs 缓存时间戳）
