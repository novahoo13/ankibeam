...
import { buildPromptFromTemplate } from "./prompt-engine.js";
    ...
    templateLibrary: {
      ...
      templates: {
        tpl_001: {
          ...
          modelName: "Basic",
          fields: [
            { name: "Front", label: "正面", parseInstruction: "単語", order: 0 },
...
function testCheckpoint3() {
  ...
  const template = {
    ...
    fields: [
      ...
      { name: "Reading", label: "読み方", parseInstruction: "発音記号", order: 1 },
      { name: "Meaning", label: "意味", parseInstruction: "日本語訳", order: 2 },
    ],
    prompt: "",
  };
  ...
  try {
    ...
    logTest("buildPromptFromTemplate()の実行", false, `エラー: ${error.message}`);
  }
}
...
function testTemplateCRUD() {
  ...
  const newTemplate = {
    ...
    modelName: "TestModel",
    fields: [
      { name: "Field1", label: "フィールド1", parseInstruction: "説明1", order: 0 },
      { name: "Field2", label: "フィールド2", parseInstruction: "説明2", order: 1 },
    ],
    prompt: "テストプロンプト",
  };
  ...
  logTest(
    "テンプレート削除",
    deleted === true && Object.keys(config.templateLibrary.templates).length === 0,
